...
* Description    : Configure USB mode
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
{
    USB_CTRL = 0x00;                                                           			// Clear USB_CTRL register
    ...
    UDEV_CTRL &= ~bUD_RECV_DIS;                                                			// Enable receiver register
	
    USB_CTRL &= ~bUC_LOW_SPEED;			
    UDEV_CTRL &= ~bUD_LOW_SPEED;                                             			// Default mode, FULL SPEED 12Mbps.
    UDEV_CTRL |= bUD_DP_PD_DIS | bUD_DM_PD_DIS;                                			// Disable DM/PM PULL_DOWN.
    UDEV_CTRL |= bUD_PORT_EN;                                                  			// Enable USB port.
}
/*******************************************************************************
* Function Name  : USBDeviceIntCfg()
* Description    : USB device init configure.
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
...
{
    USB_INT_EN |= bUIE_SUSPEND;                                               			// Enable USB device suspend interrupt.
    USB_INT_EN |= bUIE_TRANSFER;                                              			// Enable USB transmissions interrupt.
    USB_INT_EN |= bUIE_BUS_RST;                                               			// Enable USB BUS RST interrupt.
...
* Description    : Configure tie Endpoint, compound device, 
...
extern void USBDeviceEndPointCfg( void )
{
    ...
    UEP4_1_MOD |= bUEP1_TX_EN;                                                 			// Endpoint 1 send enable
    UEP4_1_MOD &= ~bUEP1_RX_EN;                                                			// Endpoint 1 receive disable
    UEP4_1_MOD &= ~bUEP1_BUF_MOD;                                              			// Endpoint 1 sigle 64 byte send buffer
    ...
    UEP2_3_MOD &= ~bUEP2_BUF_MOD;                                              			// Endpoint 2 sigle 64 byte send buffer OUT[64]+IN[64] (OUT first)
    ...
    UEP4_1_MOD &= ~(bUEP4_RX_EN | bUEP4_TX_EN);                                			// Endpoint 4 disable
...
extern void HIDValueHandle( void )
{
	...
	UINT8 readFlag = 1;																			// Uploading the keys ok.
	if (rotateFlag)							
    {
		HIDCom[0] += 0x01;	
		UploadData();
