...
****************************************
*/
/* USB host example for CH559, CH559操作扫码枪，直接字符输出或者源码输出*/
...
#define	FREQ_SYS	24000000	// 系统主频20MHz
...
UINT8	CtrlGetHIDDeviceReport( void )  // 获取HID设备报表描述符,返回在TxBuffer中
{
	UINT8	s;
	UINT8	len;
...
//						需保存端点信息以便主程序进行USB传输
              ...
              s = CtrlGetHIDDeviceReport();//获取报表描述符
						  if(s == ERR_SUCCESS)
							{
				          for ( i = 0; i < 64; i++ ) printf( "x%02X ", (UINT16)( TxBuffer[i] ) );
				          printf("\n");								
              }
//						Set_Idle( );
//						需保存端点信息以便主程序进行USB传输
...
//							给HUB各端口上电,查询各端口状态,初始化有设备连接的HUB端口,初始化设备
							...
							for ( i = 1; i <= RootHubDev[RootHubIndex].GpVar; i ++ ) {  // 清除HUB各端口连接状态
								...
								if ( s != ERR_SUCCESS ) printf( "Ext-HUB Port_%1d#  clear connection error\n",(UINT16)i );  // 端口连接状态清除失败
							}
							SetUsbSpeed( 1 );  // 默认为全速
							return( ERR_SUCCESS );
						}
...
void USBKeyToASCII( )
{
 UINT8 Tmp;
//  printf( "RxBuffer %02x\n",(UINT16)RxBuffer[2] );
 if((RxBuffer[2] == 0)&&(RxBuffer[3] == 0)) return;//释放按键
 if((RxBuffer[0] == 0x20)&&(RxBuffer[2] == 0x26)) {printf("(");return;}
 if((RxBuffer[0] == 0x20)&&(RxBuffer[2] == 0x27)) {printf(")");return;}
 if(RxBuffer[2] == 0x27) {printf("0");return;}	
 if((RxBuffer[2] >= 0x1E)&&(RxBuffer[2] <= 0x26))
 {
   Tmp = RxBuffer[2]-0x1E+'1';
   printf("%c",Tmp);return;
 }	 
}
main( ) {
